...
system_prompt.py
=================
The Dobaeni chat agent's instructions. Imported by app.py and prepended to
every conversation as the system message.

Keep this in its own file so the "instructions" half of this feature (what
the model should do) stays easy to review/tune separately from the "wiring"
half (how tool calls are executed) in app.py.
"""
